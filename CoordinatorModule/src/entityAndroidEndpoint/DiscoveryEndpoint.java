...
import entityAndroid.Discovery;
...
public class DiscoveryEndpoint {
	...
	public Response findById(@PathParam("id") final Long id) {
		//TODO: retrieve the discovery
		Discovery discovery = new Discovery();
		
		discovery.addEstablishments("001", "LocalName1", "NULL");
		discovery.addEstablishments("002", "LocalName2", "NULL");
		discovery.addEstablishments("003", "LocalName3", "NULL");
		...
		JsonObject result = null;
		 ...
		 for(int i = 0; i < array.size() && counter < 5; i++) {
			 ...
			 if(0==rand.nextInt(5)){
				 ...
				 discovery.addGenres(idTag, nameTag);
				 counter++;
			 }
		 }
			...
			JsonObject trackJson = null;
			 ...
			 for(int i = 0; i < array.size() && counter < 5; i++) {
				 ...
				 if(0==rand.nextInt(5)){
					...
					discovery.addSongs(idTrack, nameTrack, image);
					counter++;
				 }
			 }

		
		return Response.ok(discovery.toJSON()).build();
	}
